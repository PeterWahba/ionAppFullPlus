...
@ViewChild('map') mapElement: ElementRef;
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public modalCtrl: ModalController, public loadingCtrl: LoadingController,
       public viewCtrl: ViewController, private geolocation: Geolocation, public db: DatabaseProvider
    ) {
    ...
    db.getDocumentsByName('map',this.mapId)
        .then(function (e) {
            if (!!e) {
                console.log(e);
                that.mapDetail = Observable.of(e);
                that.image = e.image;
                that.title = e.title;
  ...
  calculateAndDisplayRoute() {
      ...
      this.geolocation.getCurrentPosition().then((position) => {
            ...
            loadingPopup.dismiss();
            alert("User denied Geolocation " + JSON.stringify(err))
          console.log(err);
